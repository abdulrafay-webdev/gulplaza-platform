...
def get_orders(session: Session, shop_id: Optional[int] = None, customer_id: Optional[str] = None) -> List[Order]:
    ...
    if customer_id:
        query = query.where(Order.customer_clerk_id == customer_id)
...
def get_order_by_id(session: Session, order_id: int) -> Optional[Order]:
    return session.exec(select(Order).options(selectinload(Order.items).selectinload(OrderItem.product)).where(Order.id == order_id)).first()
