...
def list_all_platform_users(user = Depends(get_super_admin), session: Session = Depends(get_session)):
    ...
    for u in seller_users:
        ...
        sellers_list.append({
            ...
            "phone": u.phone or (shop.phone if shop else "N/A"),
            "role": u.role,
            "is_active": u.is_active,
            "shop_id": shop.id if shop else u.shop_id,
            "shop_name": shop.name if shop else "No Store Attached",
            "shop_category": shop.category if shop else "General",
            "shop_address": shop.address if shop else "Karachi, Pakistan",
    ...
    for sh in all_shops:
        if sh.id not in existing_shop_ids:
            sellers_list.append({
                ...
                "phone": sh.phone or "N/A",
                "role": "SELLER",
                "is_active": sh.is_active,
                "shop_id": sh.id,
                "shop_name": sh.name,
                "shop_category": sh.category or "General",
                "shop_address": sh.address or "Karachi, Pakistan",
